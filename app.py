...
# 1.3GB, fine-tuned for sentence similarity — best accuracy for resume-JD matching
...
def run_pipeline(uploaded_files, job_description, nlp, tokenizer, roberta_model):
    ...
    # LR only reliable with 5+ resumes. Below that, use similarity+skill only.
    ...
    if USE_LR:
        ...
        X_all    = np.array([r["roberta_features"] for r in results])
        scaler.fit(X_all)
        # Dummy LR that returns similarity as probability
        class _DummyLR:
    ...
    for r in results:
        ...
        # Skill match score: what fraction of JD skills the candidate has
        skill_score = len(possessed) / max(len(expected_skills), 1)

        feat_scaled = scaler.transform(r["roberta_features"].reshape(1,-1))
        prob        = lr_model.predict_proba(feat_scaled)[0][1]
        if USE_LR:
            # Blend: 50% similarity + 30% skill match + 20% LR prediction
            final_score = round((sim * 0.5 + skill_score * 0.3 + float(prob) * 0.2) * 100, 2)
        else:
            # 60% similarity + 40% skill match (no LR with few resumes)
            final_score = round((sim * 0.6 + skill_score * 0.4) * 100, 2)
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("<div style='font-size:11px; color:#333; text-align:center;'>9 Modules • RoBERTa-based NLP<br/>Cosine Similarity + LR Classifier</div>", unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════
    ...
    for rank,r in enumerate(final_results,1):
        ...
        if score < 50 and r.get("role_suggestions"):
...
# ════════════════════════════════════════════════════════════
    ...
    if st.button("📄 Generate & Download PDF", use_container_width=True):
        with st.spinner("⚡ Generating PDF..."):
            ...
            story.append(Paragraph("Powered by RoBERTa-based Sentence Transformer",S("t3b",fontName="Helvetica-Oblique",fontSize=10,textColor=rl_colors.HexColor("#667eea"),alignment=TA_CENTER)))
